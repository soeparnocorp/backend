    ...
    private setupRoutes() {
        ...
        this.app.post('/channel/upload', async (c) => {
            ...
            try {
                ...
                const accessKeyId = c.env.ACCESS_KEY_ID;
                const secretAccessKey = c.env.SECRET_ACCESS_KEY;
...
// Helper function to generate presigned URL using native crypto
async function generatePresignedUrl(
    ...
    // Create signing key using native crypto
    ...
    const canonicalRequest = [
        method,
        canonicalUri,
        `X-Amz-Algorithm=${algorithm}&X-Amz-Credential=${encodeURIComponent(credential)}&X-Amz-Date=${amzDate}&X-Amz-Expires=${expiresIn}&X-Amz-SignedHeaders=${signedHeaders}`,
        canonicalHeaders,
        signedHeaders,
        payloadHash
    ].join("\n");
    ...
    // Build final URL
    const signedUrl = `https://${host}${canonicalUri}?X-Amz-Algorithm=${algorithm}&X-Amz-Credential=${encodeURIComponent(credential)}&X-Amz-Date=${amzDate}&X-Amz-Expires=${expiresIn}&X-Amz-SignedHeaders=${signedHeaders}&X-Amz-Signature=${signature}`;
    return signedUrl;
}
